// Versão Simplificada - Salvar diretamente no Supabase
// Este arquivo contém uma versão que salva diretamente no banco sem Edge Function
...
export function MetaOAuthConnect({ open, onOpenChange, channelType, onSuccess }: MetaOAuthConnectProps) {
  ...
  // Login direto via popup (sem Facebook SDK)
  const handleDirectLogin = () => {
    ...
    setTimeout(() => {
      ...
      setLoading(false);
      toast.error("Login cancelado ou timeout");
    }, 300000);
  };
  ...
  const handleSelectPage = async (page: any) => {
    ...
    try {
      ...
      // Salvar diretamente no Supabase (sem Edge Function)
        ...
        <div className="space-y-4 py-4">
          {step === 'auth' && (
            <>
              <Alert>
                <AlertDescription>
                  <strong>Método Simplificado!</strong> Esta versão salva diretamente no banco sem Edge Function.
                  Clique no botão abaixo para fazer login diretamente.
                </AlertDescription>
              </Alert>
                ...
                >
                  {loading ? (
                    ...
                    <>
                      ...
                      Conectar com Facebook (Versão Simplificada)
                    </>
                  )}
                </Button>
              </div>
