// Script para inserir organização de teste
const insertTestOrganization = async () => {
  const supabaseUrl = 'https://bjsuujkbrhjhuyzydxbr.supabase.co';
  const functionUrl = `${supabaseUrl}/functions/v1/insert-test-organization`;
  ...
  try {
    ...
    if (response.ok) {
      ...
      console.log('❌ Erro ao inserir organização:', response.status);
    }
